 	...
 	switch ($page) {
		...
		case 'pages':
			...
			if(isset($_POST['submitted']) == 1 ) {
				...
				if($r) {
					...
					$message = '<p class="alert alert-danger">Page could not be '.$action.' becasue: '.mysqli_error($dbc);
		...
		case 'users':
			...
			if(isset($_POST['submitted']) == 1) {
				...
				if($r) {
					...
					$message = '<p class="alert alert-danger">User could not be '.$action.' becasue: '.mysqli_error($dbc);
					if($verify == false) {
		...
		case 'navigation':
				...
				if($r) {
					...
					$message = '<p class="alert alert-danger">Navigation item could not be '.$action.' becasue: '.mysqli_error($dbc);
					$message .= '<p class="alert alert-warning">Query: '.$q.'</p>';
		...
		case 'settings':
				...
				if($r) {
					...
					$message = '<p class="alert alert-danger">Setting could not be '.$action.' becasue: '.mysqli_error($dbc);
					$message .= '<p class="alert alert-warning">Query: '.$q.'</p>';
